...
<template>
	<div class="_gaps_s">
		...
		<XFolder v-if="matchQuery([i18n.ts._role._options.simpleMode, 'simpleMode'])" v-model:policyMeta="policyMetaModel.simpleMode" :isBaseRole="isBaseRole" :readonly="readonly">
			<template #label>{{ i18n.ts._role._options.simpleMode }}</template>
			<template #valueText>{{ valuesModel.simpleMode ? i18n.ts.yes : i18n.ts.no }}</template>
			<template #default="{ disabled }">
				<MkSwitch v-model="valuesModel.simpleMode" :disabled="disabled">
					<template #label>{{ i18n.ts.enable }}</template>
				</MkSwitch>
			</template>
		</XFolder>
